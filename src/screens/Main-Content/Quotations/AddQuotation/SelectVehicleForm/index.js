  ...
  useEffect(() => {
    ...
    if (dataProps.singleData.view) {
      handleManufacturerChange(
        dataProps.singleData.vehicle[0].vehicleDetail.manufacturer.id,
      );
      ...
      <View style={styles.page}>
        ...
        <View style={{flexDirection: 'row'}}>
          <Dropdown
            ...
            value={
              dataProps.singleData.view
                ? dataProps.singleData.vehicle[0].vehicleDetail.manufacturer
                    .name
                : manufacturers[0] && manufacturers[0].label
            }
          ...
          <Dropdown
            ...
            label="Category"
            onChangeText={(value) => {
              handleVehicleCategory(value), setCategory(value);
            }}
            data={[
              {
                label: 'All Category',
          ...
          {!model.length ? (
            ...
            <FlatList
              ...
              renderItem={({item}) => {
                let eachModel = item;
                let modelShow =
                  eachModel.modelCode + ' - ' + eachModel.modelName;
                      ...
                      <View>
                        <TouchableOpacity
                          onPress={() => {
                            if (buttonType) {
                              if (!dataProps.singleData.view) {
                                let array = selectedModels.filter(
                                  (each) => each.id !== eachModel.id,
                                );
                          ...
                          <Image
                            source={{
                              uri: eachModel.image[0] && eachModel.image[0].url,
                            }}
