...
import { View, StyleSheet, Dimensions, TextInput, ScrollView, Alert, KeyboardAvoidingView, Platform, Picker, Linking } from 'react-native';
...
const LeadDataForm = (props) => {
  ...
  useEffect(() => {
    ...
    if (dataProps.singleData.view) {
      setFollowDate(dataProps.singleData.scheduleDate ? moment(dataProps.singleData.scheduleDate) : undefined)
      setFollowTime(dataProps.singleData.scheduleTime ? moment(dataProps.singleData.scheduleTime) : undefined)
  ...
  const triggerSMS = (quotationData) => {
    ...
    const cname = ((quotationData.customer && quotationData.customer.name) || (quotationData.proCustomer.name))
    const qtno = quotationData.quotationId;
    const vname = []
    quotationData.vehicle && quotationData.vehicle.map(each => {
      vname.push(each.vehicleDetail.modelName);
    })
    const slex = (quotationData.executive && quotationData.executive.profile.employeeName + ' - ' + quotationData.executive.phone);
    const link = quotationData.pdfWithBrochure;
    const linkWithoutBrochure = quotationData.pdfWithOutBrochure;
    const phone = ((quotationData.quotationPhone) || (quotationData.customer && quotationData.customer.contacts[0].phone) || (quotationData.proCustomer && quotationData.proCustomer.phone));
    ...
    platformApi
      .post("api/sendSms/quotation", obj)
      .then((result) => {
        const { data } = result;
        if (data.code === 200) {
  ...
  const generatePdf = (singleData, type) => {
    ...
    platformApi
      .post("api/quotation/pdfGenerate", { id })
      .then((result) => {
        let { data } = result;
        if (data.code === 200) {
          let { response } = data;
          ...
          if (response.code === 200) {
            ...
            let Location = response.pdfWithBrochure.data && response.pdfWithBrochure.data.Location;
            if (Location.substring(0, 4) !== "http")
              Location = "https://" + Location;
            singleData.pdfWithBrochure = Location;

            let Location2 = response.pdfWithoutBrochure.data && response.pdfWithoutBrochure.data.Location
            ...
            Alert.alert('Actions', `Choose an Action for ${singleData.quotationId}`,
              [
                {
  ...
  const pdfButton = () => {
    if (dataProps.singleData.pdfWithBrochure && dataProps.singleData.pdfWithOutBrochure) {
      Alert.alert('Actions', `Choose an Action for ${dataProps.singleData.quotationId}`, [
        {
          text: 'PDF With Brochure', onPress: () => {
            Linking.openURL(dataProps.singleData.pdfWithBrochure)
          }
        },
        {
          text: 'PDF Without Brochure', onPress: () => {
  ...
  const onSubmit = () => {
    ...
    if (dataProps.singleData.leadSource && dataProps.singleData.branch && dataProps.singleData.enquiryType && dataProps.singleData.remarks) {
      setStatus(true)
      if (followDate)
        ...
        .then((result) => {
          ...
          if (data.code === 200) {
            ...
            if (response.code === 200) {
              ...
              let filename = props.route.params.params.imagePath && props.route.params.params.imagePath.split('/').pop();
              let match = /\.(\w+)$/.exec(filename);
              let type = match ? `image/${match[1]}` : `image`;
              const formData = new FormData();
              props.route.params.params.imagePath &&
                formData.append('photo', { uri: props.route.params.params.imagePath, name: filename, type });
      ...
      .then((result) => {
        ...
        if (data.code === 200) {
          ...
          if (response.code === 200) {
            ...
            let Data = [];
            Data = dataProps.dataSource.filter((data) => data.id !== dataProps.singleData.id)
    ...
    <KeyboardAvoidingView enabled keyboardVerticalOffset={100} behavior={Platform.OS == "ios" ? "padding" : "height"}>
      ...
      <View style={styles.page}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Text style={{ textAlign: 'center', marginTop: '5%' }}>Lead Data Form</Text>
          <View style={styles.leadDataForm}>
            ...
            <Dropdown
              ...
              value={dataProps.singleData.view ? dataProps.singleData.branch.name : dataProps.singleData.branch || branch[0] && branch[0].label}
              containerStyle={{ width: '90%', marginLeft: 'auto', marginRight: 'auto', marginTop: '0%' }}
              onChangeText={(value) => { dataProps.singleData.branch = value }}
              data={branch}
            />
            {/* <Text style={{ marginLeft: '5%', marginTop: '0%', marginBottom: '-4%' }}>Enquiry Type</Text> */}
            <Dropdown
              disabled={dataProps.singleData.view}
              onChangeText={(value) => { dataProps.singleData.enquiryType = value }}
            ...
            <Dropdown
              ...
              onChangeText={(value) => { dataProps.singleData.leadSource = value }}
            ...
            />
            {
              dataProps.singleData.view
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginTop: 15 }}>
                  <Button onPress={() => { pdfButton() }} buttonStyle={{ backgroundColor: '#f75b5b', width: 120 }} title="PDF" />
                  <Button onPress={() => { props.navigation.navigate('Follow Up', dataProps.singleData && { quotation: dataProps.singleData.id, customer: dataProps.singleData.customer.id, image: dataProps.singleData.image }) }} buttonStyle={{ backgroundColor: '#f75b5b', width: 120 }} style={{ width: 120 }} title="Follow-up" />
                </View>
                :
                <></>
            }
            ...
            <Text style={{ marginLeft: '5.5%', marginTop: '5%', marginBottom: '5%', color: 'gray', fontSize: 11.5 }}>Scheduled Follow Up - Date</Text>
            <DatePicker
              disabled={dataProps.singleData.view}
            ...
            />
            <Text style={{ marginLeft: '5.5%', marginTop: '5%', marginBottom: '5%', color: 'gray', fontSize: 11.5 }}>Scheduled Follow Up - Time</Text>
            <DatePicker
              disabled={dataProps.singleData.view}
              style={{ width: 300 }}
            ...
            />
            <Text style={{ marginLeft: '5.5%', marginTop: '5%', marginBottom: '5%', color: 'gray', fontSize: 11.5 }}>Remarks</Text>
            <TextInput
              editable={!dataProps.singleData.view}
              multiline={true}
              numberOfLines={4}
              style={{ borderWidth: 1, margin: '8%', marginTop: '2%', borderColor: 'lightgray', backgroundColor: '#efefef' }}
              onChangeText={(text) => { setRemarks(text), dataProps.singleData.remarks = text }}
              value={remarks} />
          </View>
      ...
      <View style={{ position: 'absolute', bottom: '0%', left: '0%', width: '100%' }}>
        {dataProps.singleData.view
          ?
          <TouchableOpacity onPress={() => props.navigation.navigate('Quotations')} style={[styles.buttonCon, { backgroundColor: '#f75b5b' }]}>
            <Text style={styles.button}>CANCEL</Text>
          </TouchableOpacity>

          :
          <TouchableOpacity onPress={() => { if (!status) onSubmit() }} style={[styles.buttonCon, { backgroundColor: (status ? 'lightgray' : '#f75b5b') }]}>
