...
import { View, StyleSheet, Alert, Linking, Dimensions, ScrollView, Slider } from 'react-native';
...
const PricingForm = (props) => {
  ...
  const [checkedColor, setCheckedColor] = useState(dataProps.singleData.checkedColor)
  const [insurance, setInsurance] = useState(dataProps.singleData.insuranceSelected)
  const [dataTable, setDataTable] = useState(dataProps.singleData.priceTable);
  const [vehicleColor, setVehicleColor] = useState(dataProps.singleData.vehicleColor);
  ...
  const onNext = () => {
    ...
    insurance.map((eachVehicleInsurance, index) => {
      ...
      if (dataProps.singleData.vehicle[index].price.showroomPrice) {
        onRoad = onRoad + dataProps.singleData.vehicle[index].price.showroomPrice
      }
      ...
      if (dataProps.singleData.vehicle[index].price.registrationFee) {
        onRoad = onRoad + dataProps.singleData.vehicle[index].price.registrationFee
      }
      if (dataProps.singleData.vehicle[index].price.warrantyPrice) {
        onRoad = onRoad + dataProps.singleData.vehicle[index].price.warrantyPrice
      }
      if (dataProps.singleData.vehicle[index].price.handlingCharges) {
        onRoad = onRoad + dataProps.singleData.vehicle[index].price.handlingCharges
      }
      eachVehicleInsurance.map((element, i) => {
        if (element && i === 0) {
          array.push({
            ...
            onRoad: onRoad + dataProps.singleData.vehicle[index].price.insurance1plus5
          })
        }
        if (element && i === 1) {
          array.push({
            ...
            onRoad: onRoad + dataProps.singleData.vehicle[index].price.insurance5plus5
          })
        }
        if (element && i === 2) {
          array.push({
            ...
            onRoad: onRoad + dataProps.singleData.vehicle[index].price.insurance1plus5ZD
          })
        }
        if (element && i === 3) {
          array.push({
            ...
            onRoad: onRoad + dataProps.singleData.vehicle[index].price.insurance5plus5ZD
          })
        }
      })
  ...
  return (
    <View style={styles.page}>
      <Text style={{ textAlign: 'center', marginTop: '3%', marginBottom: '3%' }}>Pricing Form</Text>
      <View style={styles.pricingForm}>
        {!selectedVehicle.length
          ?
          <Text style={styles.noModels}>Select Vehicles in Previous Page</Text>
          :
          <ScrollView  showsVerticalScrollIndicator ={false} style={{ marginLeft: 8, marginRight: 8 }}>
            {selectedVehicle.map((eachSelected, index) => {
              return (
                <View style={{ marginTop: 15 }}>
                  <View>
                    <Card >
                      <Text style={[styles.modelTitle, { marginBottom: '4%' }]}>{eachSelected.modelCode} - {eachSelected.modelName}</Text>
                      <View style={{ display: 'flex', flexDirection: 'row' }}>
                        ...
                        />
                        <Image
                          source={{ uri: eachSelected.image[vehicleColor[index]].url }}
                      ...
                      <Text style={[styles.modelTitle, { marginBottom: '4%' }]}>{eachSelected.image[vehicleColor[index]].color}</Text>
                      ...
                      {!dataProps.singleData.view ?
                        <Card >
                          ...
                          {!dataProps.singleData.view && eachSelected.price[0] && eachSelected.price[0].insurance1plus5 ?
                            <CheckBox
                              center
                              checkedColor='#f75b5b'
                              ...
                              onPress={() => {
                                ...
                                if (!insurance[index][0]) {
                                  ...
                                  table[index].map(each => {
                                    if (each[0] !== 'On-Road Price(1 + 5)' && each[0] !== 'On-Road Price(5 + 5)' && each[0] !== 'On-Road Price(1 + 5)ZD' && each[0] !== 'On-Road Price(5 + 5)ZD')
                                      total = total + each[1];
                                  })
                                  total = total + eachSelected.price[0].insurance1plus5
                                  table[index].push(['On-Road Price(1 + 5)', total])
                                }
                                else {
                                  table[index] = table[index].filter(each => { return each[0] !== "On-Road Price(1 + 5)" })
                                }
                                setDataTable([...table])
                          ...
                          {!dataProps.singleData.view && eachSelected.price[0] && eachSelected.price[0].insurance5plus5 ?
                            <CheckBox
                              center
                              checkedColor='#f75b5b'
                              ...
                              onPress={() => {
                                ...
                                if (!insurance[index][1]) {
                                  ...
                                  total = total + eachSelected.price[0].insurance5plus5
                                  table[index].push(['On-Road Price(5 + 5)', total])
                                }
                                else {
                                  table[index] = table[index].filter(each => { return each[0] !== "On-Road Price(5 + 5)" })
                                }
                                setDataTable([...table])
                            ...
                            /> : <Text style={{ marginTop: -14 }}></Text>}
                          {!dataProps.singleData.view && eachSelected.price[0] && eachSelected.price[0].insurance1plus5ZD ?
                            <CheckBox
                              center
                              checkedColor='#f75b5b'
                              ...
                              onPress={() => {
                                ...
                                if (!insurance[index][2]) {
                                  ...
                                  total = total + eachSelected.price[0].insurance1plus5ZD
                                  table[index].push(['On-Road Price(1 + 5)ZD', total])
                                }
                                else {
                                  table[index] = table[index].filter(each => { return each[0] !== "On-Road Price(1 + 5)ZD" })
                                }
                                setDataTable([...table])
                            ...
                            /> : <Text style={{ marginTop: -14 }}></Text>}
                          {!dataProps.singleData.view && eachSelected.price[0] && eachSelected.price[0].insurance5plus5ZD ?
                            <CheckBox
                              checkedColor='#f75b5b'
                              center
                              ...
                              onPress={() => {
                                ...
                                if (!insurance[index][3]) {
                                  ...
                                  total = total + eachSelected.price[0].insurance5plus5ZD
                                  table[index].push(['On-Road Price(5 + 5)ZD', total])
                                }
                                else {
                                  table[index] = table[index].filter(each => { return each[0] !== "On-Road Price(5 + 5)ZD" })
                                }
                                setDataTable([...table])
                      ...
                      <View style={styles.container}>
                        <Table borderStyle={{ borderColor: 'transparent' }}>
                          <Row data={['Type', 'Price']} flexArr={[2, 1]} style={styles.head} textStyle={styles.text} />
                          <TableWrapper style={styles.wrapper}>
                            {/* <Col data={tableTitle} style={styles.title} heightArr={[28, 28]} textStyle={styles.text} /> */}
                            <Rows data={dataTable[index]} flexArr={[2, 1]} style={styles.row} textStyle={styles.text} />
                          </TableWrapper>
                        </Table>
                      </View>
                    </Card>
